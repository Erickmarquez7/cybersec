# My Road Map - Fundamentos

Recopilatorio de salas **gratuitas** teoricas y prácticas en Try Hack Me. Unicamente se necesita tener cuenta en la página.

## General

- Representación de datos: <https://tryhackme.com/room/datarepresentation>
- Codificación de datos: <https://tryhackme.com/room/dataencoding>

### Conceptos básicos de seguridad

- Principios de seguridad: <https://tryhackme.com/room/securityprinciples>

- Criptografía básica: <https://tryhackme.com/room/cryptographyconcepts>
- Criptografía: <https://tryhackme.com/room/cryptographybasics>
- Encriptación: https://tryhackme.com/room/encryptioncrypto101

- Tríada de la seguridad: <https://tryhackme.com/room/theciatriad>

### Sistemas Operativos

- Introducción a los Sistemas operativos: <https://tryhackme.com/room/operatingsystemsintroduction>

- Conexión a THM a través de OpenVPN: <https://tryhackme.com/room/openvpn>

#### Linux

- Comandos útiles: <https://tryhackme.com/room/linuxmodules>

- Practica guiada: <https://tryhackme.com/room/linuxcli-aoc2025-o1fpqkvxti>

#### Windows

- Windows Basic: <https://tryhackme.com/room/windowsbasics>

Las siguientes 3 salas, junto con Active Directory basics, forman parte del módulo "Windows and AD Fundamentals"

- Windows 1: <https://tryhackme.com/room/windowsfundamentals1xbx>
- Windows 2: <https://tryhackme.com/room/windowsfundamentals2x0x>
- Windows 3: <https://tryhackme.com/room/windowsfundamentals3xzx>

- Linea de comandos: <https://tryhackme.com/room/windowscommandline>

##### Active Directory

- Básico: <https://tryhackme.com/room/winadbasics>


### Base de datos


### Red

- Conceptos de red: <https://tryhackme.com/room/networkingconcepts>
- ¿Qué es una red?: <https://tryhackme.com/room/whatisnetworking>
- Introducción al modelo LAN: <https://tryhackme.com/room/introtolan>
- Introducción a la red: <https://tryhackme.com/room/introtonetworking>

#### Protocolos comunes

- Recomendable nmap: <https://tryhackme.com/room/furthernmap>
- Reto guiado: <https://tryhackme.com/room/networkservices>
- Segunda parte: <https://tryhackme.com/room/networkservices2>

#### Web

Las siguientes 4 salas forman parte del módulo "How the web works"

- Domain Name System (DNS): <https://tryhackme.com/room/dnsindetail>
- HyperText Transfer Protocol (HTTP): <https://tryhackme.com/room/httpindetail>
- Cómo funciona una pagina web: <https://tryhackme.com/room/howwebsiteswork>
- Todo junto: <https://tryhackme.com/room/puttingitalltogether>

La siguiente sala puede servir para profundizar y repasar
- Pagina web: <https://tryhackme.com/room/webapplicationbasics>

- Seguridad web (IDOR): <https://tryhackme.com/room/introwebapplicationsecurity>

### Herramientas útiles

- Habilidades de búsqueda: <https://tryhackme.com/room/searchskills>
- Google dorking: <https://tryhackme.com/room/googledorking>
- Cyberchef: <https://tryhackme.com/room/cyberchefbasics>
- Shodan: <https://tryhackme.com/room/shodan>
- Aircrack: https://tryhackme.com/room/wifihacking101

- Reto guiado: <https://tryhackme.com/room/missingperson>

### Ofensivo

- Introducción: https://tryhackme.com/room/offensivesecurityintro

#### Fuerza bruta
- Hydra básico: <https://tryhackme.com/room/hydra>

#### Enumeración

- Reto guiado: <https://tryhackme.com/room/contentdiscoveryx>

#### OWASP (2025)
Las siguientes salas forman parte del modulo OWASP Top 10 (2025) <https://tryhackme.com/module/owasp-top-10-2025>

- OWASP Top 10 2025: IAAA Failures <https://tryhackme.com/room/owasptopten2025one>
- OWASP Top 10 2025: Application Design Flaws <https://tryhackme.com/room/owasptopten2025two>
- OWASP Top 10 2025: Insecure Data Handling <https://tryhackme.com/room/owasptopten2025three>


### Defensivo

- Introducción: https://tryhackme.com/room/defensivesecurityintro


#### SIEM
- Introducción a SIEM: <https://tryhackme.com/room/introtosiem>

#### SOC
- Fundamentos de SOC: <https://tryhackme.com/room/socfundamentals>

## Jr pentest

### Reconocimiento y enumeración

- Reconocimiento pasivo: <https://tryhackme.com/room/passiverecon>
- Reconocimiento activo: <https://tryhackme.com/room/activerecon>

- nmap I: <https://tryhackme.com/room/nmap01>
- nmap II: <https://tryhackme.com/room/nmap02>
- nmap: <https://tryhackme.com/room/furthernmap>

- Reto: <https://tryhackme.com/room/takeover>

### Explotación

- IDOR guiado: <https://tryhackme.com/room/idor-aoc2025-zl6MywQid9>
- Reto neighbour: <https://tryhackme.com/room/neighbour>
- Reto corridor: <https://tryhackme.com/room/corridor>

## Jr SOC